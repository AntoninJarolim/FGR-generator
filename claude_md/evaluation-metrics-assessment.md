...
7. **The small-model JSON-prompt runs are superseded** by their XML span-grammar runs (28.7–50.4% non-verbatim) and archived in `long-embed-json-superseded/` — excluded from metrics. `glm-5.2` is NOT among them: it is the sole GLM run (e-INFRA API, JSON-only, no XML counterpart) and reads reliably even unconstrained, so it lives in `long-embed-json/` and IS evaluated (a few records carry non-string spans, which the metric code skips). (Note: the JSON-*constrained* large-model runs in `long-embed-json-constrained` are a legacy arm — produced by the now-removed `--vllm_guided_json` option, which constrained the JSON schema but not verbatimness; kept for now, not regenerable.)
